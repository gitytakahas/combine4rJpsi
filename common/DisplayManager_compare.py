...
class DisplayManager(object):
    ...
    def __init__(self, name, ratio):
        ...
        self.histos = []
        self.Legend = ROOT.TLegend(0.5, 0.65, 0.9, 0.89)
        applyLegendSettings(self.Legend)

        self.draw_ratioLegend = ROOT.TLegend(0.5, 0.7, 0.9, 0.89)
...
#        self.canvas.Print(self.name + ']')
    ...
    def Draw(self, histos, titles, textitle, xmax=None):
        ...
        self.Legend.AddEntry(self.histos[-1], textitle, '')

        for i, h in enumerate(self.histos):
            title = titles[i]
#            h.GetYaxis().SetRangeUser(0.1, ymax * 1.5)
            h.GetYaxis().SetRangeUser(0.01, ymax * 1.5)
            if xmax:
                h.GetXaxis().SetRangeUser(0., xmax)
            self.Legend.AddEntry(h, title + ': ' + str(int(h.Integral())), 'lep')
        ...
        if self.draw_ratio:
            ...
            for ihist in range(1, len(self.histos)):
                ...
                self.draw_ratioLegend.AddEntry(histPull, titles[ihist], 'lep')


                line = ROOT.TLine(histPull.GetXaxis().GetXmin(), 1,
                             histPull.GetXaxis().GetXmax(), 1)
...
#        self.canvas.Print(self.name.replace('.pdf','_log.gif'))
